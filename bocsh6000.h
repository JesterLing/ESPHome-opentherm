...
class BoilerDHWSwitch : public Switch, public Component {
public: 
...
class Boiler : public Component, public climate::Climate, public CustomAPIDevice {
private:
  // timer for OT
  unsigned long now_m;
  // timer for pause after response
  unsigned long now_m_last;
...
public:
  ...
  void setup() override {
    ...
    now_m_last = (millis() + 15000); // first wait 15 sec before start ot
    timeout_count = 0;
    request = 0;
    ...
    this->boiler_water_temp->add_filter(new SlidingWindowMovingAverageFilter(3, 3, 1));
    ...
    this->dhw_temperature->add_filter(new SlidingWindowMovingAverageFilter(3, 3, 1));
  ...
  void loop() override {
	...
	now_m = millis();
	if((now_m_last < now_m) && (now_m - now_m_last) > 700) {
  ...
  void temp_calculation() {
	...
	if(difference > 0 && !t_wait) {
        if(this->heat_interval->state != "0") this->heat_interval->publish_state("0");
            ...
            if(chTempSet >= 40) {
               	if(this->boiler_water_temp->state >= chTempSet && this->flame->state) {
                  temp_reach = true;
                }
            }
    } else {
 ...
 void HandleReply(uint32_t response) {
    ...
    case OpenThermMessageID::ASFflags: {
          ...
          // } else { (flags & 0x02) - lockout_reset (удаленный сброс?)
    ...
    case OpenThermMessageID::MaxTSetUBMaxTSetLB:
        // макс темп = (response & 0xFFFF) >> 8;
        // мин темп = response & 0xFF;
    default:
      break;
    }
  }
