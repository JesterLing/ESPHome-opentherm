...
namespace opentherm {
	...
	void OpenThermGWClimate::setup()
	{
		...
		this->mode_select->add_on_state_callback([this](const std::string& value, size_t index) { ESP_LOGD(TAG, "Working mode changed. index %i", index); });
	...
	void IRAM_ATTR OpenThermGWClimate::handleInterrupt() { ot.handleInterrupt(); }
	...
	{
		if(!this->mode_select->has_state() || this->mode == climate::CLIMATE_MODE_OFF || !esphome::network::is_connected() || !remote_is_connected())
			return 0.0f;

		if(this->mode_select->index_of("Автоматический PID") == this->mode_select->active_index()) {
			return this->pid_output->get_state() * 85;
		} else if(this->mode_select->index_of("Ручной") == this->mode_select->active_index()) {
			float ch_temp = 0;
			...
			if(this->interval_wait) { // if now is a waiting interval, count the time until the end of the interval
				...
				if(this->heat_interval != nullptr && this->heat_interval->state != "0")
					this->heat_interval->publish_state("0");
				if(!std::isnan(this->current_temperature)) { // if we have a room temperature sensor decide heat or not
					float difference = this->target_temperature - this->current_temperature;
					if(difference > 0)
						this->heatup = true;
					if(difference <= 0 && (!this->heatup || difference < -1.5)) {
						ch_temp = 0;
						this->heatup = false;
					}
				}
			...
			ESP_LOGD(TAG, "CH_TEMP: %f", ch_temp);
			return ch_temp;
		} else {
			return 0.0f;
		}
	...
	void OpenThermGWClimate::dump_config() { LOG_CLIMATE("", "OpenTherm Gateway Climate", this); }
	...
	void OpenThermGWClimate::processResponse(uint32_t response, OpenThermResponseStatus status)
	{
		...
		case OpenThermResponseStatus::TIMEOUT:
			if(this->timeout_count < 10) {
				...
				ESP_LOGE(TAG,
					"There is no communication with the boiler via the OT bus, "
	...
	void OpenThermGWClimate::process_Status(uint32_t& response)
	{
		...
		if(this->flame_on->state != slave_flame_on || !this->flame_on->has_state())
		...
		if(slave_fault_indication) {
			...
			if(this->fault != nullptr && this->fault->state != "OK")
				this->fault->publish_state("OK");
		}
	...
	void OpenThermGWClimate::request_Status(bool ch, bool dhw) { ot.sendRequestAync(ot.buildSetBoilerStatusRequest(ch, dhw, false, false, false)); }

	void OpenThermGWClimate::request_TSet(float temp) { ot.sendRequestAync(ot.buildSetBoilerTemperatureRequest(temp)); }

	void OpenThermGWClimate::request_Tboiler() { ot.sendRequestAync(ot.buildGetBoilerTemperatureRequest()); }

	void OpenThermGWClimate::request_Tdhw() { ot.sendRequestAync(ot.buildRequest(OpenThermMessageType::READ_DATA, OpenThermMessageID::Tdhw, 0)); }

	void OpenThermGWClimate::request_DHWFlowRate() { ot.sendRequestAync(ot.buildRequest(OpenThermRequestType::READ_DATA, OpenThermMessageID::DHWFlowRate, 0)); }

	void OpenThermGWClimate::request_RelModLevel() { ot.sendRequestAync(ot.buildRequest(OpenThermRequestType::READ_DATA, OpenThermMessageID::RelModLevel, 0)); }

	void OpenThermGWClimate::request_ASFflags() { ot.sendRequestAync(ot.buildRequest(OpenThermRequestType::READ_DATA, OpenThermMessageID::ASFflags, 0)); }

	void OpenThermGWClimate::request_Toutside() { ot.sendRequestAync(ot.buildRequest(OpenThermRequestType::READ_DATA, OpenThermMessageID::Toutside, 0)); }

	void OpenThermGWClimate::request_CHPressure() { ot.sendRequestAync(ot.buildRequest(OpenThermRequestType::READ_DATA, OpenThermMessageID::CHPressure, 0)); }

	void OpenThermGWClimate::request_Tret() { ot.sendRequestAync(ot.buildRequest(OpenThermRequestType::READ_DATA, OpenThermMessageID::Tret, 0)); }
